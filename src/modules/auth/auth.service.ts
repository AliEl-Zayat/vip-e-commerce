...
export class AuthService {
  ...
  async generateQRSession(): Promise<{ sessionId: string; qrToken: string; qrCodeUrl: string }> {
  ...
  async authenticateQR(data: AuthenticateQRDto, userId: string): Promise<void> {
  ...
  async scanQR(sessionId: string, qrToken: string): Promise<void> {
    const session = await QRSession.findOne({
      sessionId,
      qrToken,
    });
